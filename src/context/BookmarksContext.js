...
export const BookmarksContextProvider = ({ children }) => {
    ...
    const tagsFunc = {
        ...
        deleteTags: async function () {
            totTags.map(async ttd => {
                if (ttd.ischeck === true) {
                    const response = await fetch('https://kind-rose-angelfish-cape.cyclic.app/api/tags/' + ttd._id, {
                        method: 'DELETE',
                        headers: {
    ...
    const bookFunc = {
        addBookmark: async function (bookToAdd) {
            ...
            const response = await fetch('https://kind-rose-angelfish-cape.cyclic.app/api/bookmarks', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
        ...
        modifyBookmark: async function (bookUpdated) {
            ...
            const response = await fetch('https://kind-rose-angelfish-cape.cyclic.app/api/bookmarks/' + bookUpdated._id, {
                method: 'PATCH',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${user.token}`
            ...
            if (response.ok) {
                ...
                const upd = await fetch('https://kind-rose-angelfish-cape.cyclic.app/api/bookmarks/', {
                    method: 'GET',
                    headers: {
                        'Content-Type': 'application/json',
                        'Authorization': `Bearer ${user.token}`
        ...
        deleteBookmark: async function (bookToDel) {
            const response = await fetch('https://kind-rose-angelfish-cape.cyclic.app/api/bookmarks/' + bookToDel._id, {
                method: 'DELETE',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${user.token}`
